...
public class RequestMappingHandlerMapping {
  ...
  public static class RequestHandler {
    ...
    public Object invoke(HttpServletRequest request, HttpServletResponse response) throws Exception {
      ...
      // request handler의 작업 결과물과 JSP URL을 담은 맵 객체를 리턴한다.
      return model;
    }
    ...
    private List<String> getParameterNames(Class<?> type, Method method) throws Exception {
      ...
      // => 그러나 .class 파일에는 분명히 파라미터 이름이 들어 있다.
    ...
    private Object getPojoValue(String paramName, Class<?> paramType, HttpServletRequest request) throws Exception {
      ...
      // 세터 메서드를 추출한다.
      Set<Method> methods = ReflectionUtils.getMethods(paramType, withPrefix("set"));
    ...
    private Object getPrimitiveValue(String paramName, Class<?> paramType, HttpServletRequest request) {
      ...
      if (paramType == byte.class || paramType == Byte.class) return Byte.parseByte(value);
      else if (paramType == short.class || paramType == Short.class) return Short.parseShort(value);
      else if (paramType == int.class || paramType == Integer.class) return Integer.parseInt(value);
      else if (paramType == long.class || paramType == Long.class) return Long.parseLong(value);
      else if (paramType == float.class || paramType == Float.class) return Float.parseFloat(value);
      else if (paramType == double.class || paramType == Double.class) return Double.parseDouble(value);
      else if (paramType == char.class || paramType == Character.class) return value.charAt(0);
      else if (paramType == boolean.class || paramType == Boolean.class) return Boolean.parseBoolean(value);
      else if (paramType == java.util.Date.class || 
          paramType == java.sql.Date.class) return java.sql.Date.valueOf(value); // 문자열 형식 : "yyyy-MM-dd" 이어야 한다.

      return value;
    }

    private boolean isPrimitiveType(Class<?> paramType) {
      if (paramType == byte.class || paramType == Byte.class || paramType == short.class || paramType == Short.class 
          || paramType == int.class || paramType == Integer.class || paramType == long.class || paramType == Long.class 
          || paramType == float.class || paramType == Float.class || paramType == double.class || paramType == Double.class 
          || paramType == char.class || paramType == Character.class || paramType == boolean.class || paramType == Boolean.class 
          || paramType == String.class || paramType == java.util.Date.class || 
