...
public class Test14 {
  ...
  static void findClass(File dir, String packageName) {
    // 1) dir가 파일이면 패키지 이름과 파일 이름을 합쳐 출력하고 리턴한다.
    //    => 단 파일 이름에서 .class 확장자 명은 제외한다.
    //    => 파일 명이 Hello.class 이고 패키지명이 aaa.bbb 라면
    //       출력할 이름은 aaa.bbb.Hello 이다.
    if(dir.isFile()) {
      //String name = String.format("%s.%s", packageName, dir.getName().replace(".class", ""));

      System.out.println(
          String.format("%s.%s", packageName, dir.getName().replace(".class", "")).substring(1));
      return;
    }

    // 2) dir가 디렉토리라면 하위 디렉토리와 파일 목록을 얻는다.
    //    => 단 필터를 이용하여 디렉토리와 클래스 파일(.class) 목록만 추출한다.
    File[] files = dir.listFiles(path -> path.isDirectory() || path.getName().endsWith(".class"));
  ...
  static void findClass2(File dir, String packageName) {
