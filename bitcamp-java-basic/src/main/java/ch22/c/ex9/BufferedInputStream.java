...
public class BufferedInputStream extends InputStream {
  ...
  public int read() throws IOException {
    if (cursor >= size) { // 버퍼에 보관된 데이터를 다 읽었으면,
      count++;
      size = in.read(buf); // 다시 FileInputStream을 사용하여 1024 바이트를 읽어온다.
      if (size == -1) // 물론 파일에 읽은 데이터가 없다면, 즉 다 읽었다면 -1을 리턴한다.
        return -1;
      cursor = 0; // FileInputStream을 사용하여 새로 1024 바이트를 읽어 버퍼에 저장했따면,
    }             // 다시 커서의 위치를 0으로 설정한다.

    // 배열에 들어 있는 바이트의 값이 양수일 경우 int로 리턴하면 그 값 그대로 리턴된다.
    // 예) 0x7f(01001111, 10진수로 127) 값을 리턴한다면 
    //     4바이트 int 값 0x0000007f가 리턴된다.
    ...
    // 예) 0x80 --> 0x00000080 & 0x000000ff --> 0x00000080
    //
