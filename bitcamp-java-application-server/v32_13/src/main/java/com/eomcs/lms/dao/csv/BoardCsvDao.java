...
public class BoardCsvDao extends AbstractCsvDataSerializer<Board, Integer> {
  ...
  // 수퍼 클래스에서 template mathod를(loadSave())를 정의하고
  ...
  protected Board createObject(String[] values) {
    ...
    board.setCreateDate(Date.valueOf(values[2]));
  ...
  protected String createCSV(Board obj) {
    return String.format("%d,%s,%s,%d", obj.getNo(), obj.getContents(), obj.getCreateDate(),
        obj.getViewCount());
  }
