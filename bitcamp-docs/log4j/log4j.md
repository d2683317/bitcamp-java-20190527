...
담당자 이름은 마음대로 작성한다.
이 담당자 이름으로 이름으로 출력 방법을 자세하게 정의한다.
...
예) log4j.appender.aaa=org.apache.log4j.ConsoleAppender <-- 콘솔 창으로 출력
예) log4j.appender.aaa=org.apache.log4j.FileAppender <-- 파일로 출력
예) log4j.appender.aaa=org.apache.log4j.net.SocketAppender <-- 원격 서버로 출력
```
...
```
log4j.appender.출력담당자명.layout=패키지명을 포함한 클래스
예)
log4j.appender.aaa.layout=org.apache.log4j.SimpleLayout (한 줄의 간단한 문자열 출력)
log4j.appender.aaa.layout=org.apache.log4j.HYMLLayout (HTML 테이블 혀식으로 출력)
