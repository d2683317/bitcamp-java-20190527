  ...
  private static void loadMemberData() {
    ...
    try {
      ...
      while (scan.hasNextLine()) {
        ...
        member.setResisteredDate(Date.valueOf(data[6]));
    ...
    try {
      ...
      for (Member member : memberList) {
        ...
        String str = String.format("%d,%s,%s,%s,%s,%s,%s\n", member.getNo(),
            member.getName(), member.getEmail(), member.getPw(), member.getPhoto(),
  ...
  private static void loadBoardData() {
    ...
    try {
      ...
      while (scan.hasNextLine()) {
        ...
        board.setCreateDate(Date.valueOf(data[3]));
    ...
    try {
      ...
      for (Board board : boardList) {
        ...
        String str = String.format("%d,%s,%d,%s\n", board.getNo(),
            board.getContents(), board.getViewCount(), board.getCreateDate());
        out.write(str);
      }
    } catch (FileNotFoundException e) {
      // 출력할 파일을 생성하지 못할 때
